document.addEventListener("DOMContentLoaded",()=>{
  ...
  if(featured&&window.BBR_PRODUCTS){
    featured.innerHTML=window.BBR_PRODUCTS.filter(p=>p.featured).map(p=>`
      ...
      <span class="badge">${p.badge}</span><h3>${p.name}</h3><p>${p.description}</p>
      <div class="price-row"><span class="price">${p.pricePKR}</span><span class="price">${p.priceUSD}</span></div>
      <a class="btn-small" href="${productUrl(p.code)}">View Product</a></div></article>`).join("");
  }
  ...
  if(detail&&window.BBR_PRODUCTS){
    ...
    const seoDescription=`${p.name} — ${p.description} Handmade by Beads by Rumi. Shop handcrafted beaded bags and accessories from Pakistan.`;
    ...
    setMeta("og:type","product");setMeta("og:title",`${p.name} | Beads by Rumi`);setMeta("og:description",seoDescription);setMeta("og:url",canonical.href);setMeta("og:image",new URL(p.image,location.href).href);setMeta("og:site_name","Beads by Rumi");
    setMeta("twitter:card","summary_large_image","name");setMeta("twitter:title",`${p.name} | Beads by Rumi`,"name");setMeta("twitter:description",seoDescription,"name");setMeta("twitter:image",new URL(p.image,location.href).href,"name");
    const existingSchema=document.querySelector('script[data-product-schema]');if(existingSchema)existingSchema.remove();
    const schema=document.createElement("script");schema.type="application/ld+json";schema.setAttribute("data-product-schema","true");
    const pricePKR=Number(String(p.pricePKR||"").replace(/[^\d.]/g,""));
    const availability=String(p.status||"").toLowerCase().includes("in stock")?"https://schema.org/InStock":"https://schema.org/PreOrder";
    schema.textContent=JSON.stringify({"@context":"https://schema.org","@type":"Product","name":p.name,"description":p.description,"image":[new URL(p.image,location.href).href],"sku":p.code,"category":p.category,"brand":{"@type":"Brand","name":"Beads by Rumi"},"offers":{"@type":"Offer","url":canonical.href,"priceCurrency":"PKR","price":pricePKR,"availability":availability}});
    document.head.appendChild(schema);
    detail.innerHTML=`<img src="${p.image}" alt="${p.name}"><div><span class="badge">${p.badge}</span><p class="eyebrow">${p.collection} · ${p.category}</p><h1>${p.name}</h1><div class="price-row"><span class="price">${p.pricePKR}</span><span class="price">${p.priceUSD}</span></div><p class="description">${p.description}</p><div class="info-list"><div><strong>Product Code</strong><span>${p.code}</span></div><div><strong>Status</strong><span>${p.status}</span></div><div><strong>Production</strong><span>${p.production}</span></div><div><strong>Materials</strong><span>${p.materials}</span></div><div><strong>Colors</strong><span>${p.colors}</span></div><div><strong>Bulk Orders</strong><span>${p.bulk}</span></div></div><a class="btn" href="https://wa.me/923322636648?text=${encodeURIComponent("Hi Beads by Rumi, I’d like to order "+p.name+" ("+p.code+").")}">Order on WhatsApp</a></div>`;
  }
});
